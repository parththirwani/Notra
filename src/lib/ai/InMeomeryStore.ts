...
export class RedisStore {
  ...
  async add(conversationId: string, message: Message): Promise<Message[]> {
    ...
    if (cached) {
      ...
      console.log(
        `[RedisStore] No cache found. Loading from database for ${conversationId}`
      );
      ...
      console.log(
        `[RedisStore] Loaded ${messages.length} messages from database`
      );
    }
    ...
    console.log(
      `[RedisStore] Messages cached in Redis for ${conversationId} with TTL: ${CACHE_TTL}s`
    );
  ...
  async get(conversationId: string): Promise<Message[]> {
    ...
    if (cached) {
      console.log(
        `[RedisStore] Messages found in Redis cache for ${conversationId}`
      );
      ...
      console.log(
        `[RedisStore] Returning ${messages.length} cached messages`
      );
      return messages;
    }
    ...
    console.log(
      `[RedisStore] No cache found. Loading conversation ${conversationId} from database`
    );
    const dbMessages = await prisma.message.findMany({
      where: { conversationId },
      orderBy: { createdAt: "asc" },
    ...
    console.log(
      `[RedisStore] Loaded ${dbMessages.length} messages from database for ${conversationId}`
    );
    ...
    if (messages.length > 0) {
      await redis.setex(key, CACHE_TTL, JSON.stringify(messages));
      console.log(
        `[RedisStore] ${messages.length} messages cached in Redis for ${conversationId}`
      );
    }
